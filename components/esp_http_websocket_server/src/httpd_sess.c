...
esp_err_t httpd_sess_process(struct httpd_data *hd, int newfd)
{
    ...
    if(!sd->is_websocket)
    {
        ESP_LOGD(TAG, LOG_FMT("not websocket"));
    ...
    {
        char buf[WEBSOCKET_BUF_SIZE]; 
        memset(buf, 0, WEBSOCKET_BUF_SIZE);
        ESP_LOGD(TAG, LOG_FMT("is websocket"));
        int ret = sd->recv_fn(sd->handle, sd->fd, buf, WEBSOCKET_BUF_SIZE, 0);
