...
/* Queue for outbound websocket messages */
...
static esp_err_t httpd_server(struct httpd_data *hd)
{
    ...
    /* If we need to write, then add all of the websocket clients to the write
     * set
     */ 
    int fd = -1;
    //short-circuit or used to get a message when we have already written the
    //one from the queue
    if(need_to_write || pdTRUE == xQueueReceive(out_queue, &s, 0))
    {
        while ((fd = httpd_sess_iterate(hd, fd)) != -1) {
    ...
    /* Case3: Are we good to send data to one of the websocket 
     * clients? */
    while ((fd = httpd_sess_iterate(hd, fd)) != -1) {
        if (FD_ISSET(fd, &write_set)) {
            /* TODO only clear the flag when all of the clients have received
             * the message */
            need_to_write = false;
            ESP_LOGD(TAG, LOG_FMT("writing %s to websocket %d"), s, fd);
            if (httpd_websocket_send_string(fd, s) != ESP_OK) {
                ESP_LOGD(TAG, LOG_FMT("closing socket %d"), fd);
                close(fd);
                /* Delete session and update fd to that
                 * preceding the one being deleted */
...
static esp_err_t httpd_server_init(struct httpd_data *hd, QueueHandle_t in, QueueHandle_t out)
{
    httpd_websocket_init(in);
    out_queue = out;
...
esp_err_t httpd_start(httpd_handle_t *handle, const httpd_config_t *config, QueueHandle_t in_q, QueueHandle_t out_q)
{
    ...
    if (httpd_server_init(hd, in_q, out_q) != ESP_OK) {
        httpd_delete(hd);
        return ESP_FAIL;
    }
