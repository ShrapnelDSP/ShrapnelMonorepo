...
std::optional<CreateRequest> from_json(const rapidjson::Value &json)
{
    ...
    auto mapping =
        from_json<std::pair<Mapping::id_t, Mapping>>(mapping_member->value);
...
std::optional<MappingApiMessage> from_json(const rapidjson::Value &json)
{
...

    {
        const etl::map<std::string,
                       std::function<std::optional<MappingApiMessage>()>,
                       4>
            lut{
                {get_message_type<GetRequest>(),
                 [&] { return from_json<GetRequest>(json); }},
                {get_message_type<CreateRequest>(),
                 [&] { return from_json<CreateRequest>(json); }},
                {get_message_type<Update>(),
                 [&] { return from_json<Update>(json); }},
                {get_message_type<Remove>(),
                 [&] { return from_json<Remove>(json); }},
            };
